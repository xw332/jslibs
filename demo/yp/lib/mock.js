...
(function(undefined) {
    ...
    function convert(item, options) {
        ...
        var cbData = Util.isFunction(item.template) ? item.template(options) : Mock.mock(item.template);
        var extData = {
            url: options.url,
            type: options.type,
            data: options.data && decodeURI(options.data)
        };
        window.console && console.log('$.ajax', extData, cbData);
        return cbData;
    }
    Mock.mockjax = function mockjax(jQuery) {
        function mockxhr() {
            return {
                ...
                send: function() {
                    var self = this///
                    self.onload && setTimeout(function() {
                        self.onload();
                    });
                },
