...
Mock.mockjax = function mockjax(jQuery) {
    ...
    function convert(mock, extData) {
        return function() {
            var cbData = Mock.mock(
                jQuery.isFunction(mock.template) ? mock.template() : mock.template
            )
            console.log('$.ajax', extData, cbData); ///
            return cbData
        }
    }
    ...
    function prefilter(options) {
        for (var surl in Mock._mocked) {
            ...
            var extData = {url:options.url,type:options.type,data:decodeURI(options.data)}///
            options.dataFilter = convert(mock, extData)///
